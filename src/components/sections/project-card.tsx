...
export function ProjectCard({ project, className = '' }: ProjectCardProps) {
  return (
    <article
      className={`group flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/10 bg-white/[0.03] ${className}`.trim()}
    >
      ...
      <div className="flex flex-1 flex-col justify-between gap-6 p-5 md:p-6">
        <div className="space-y-3">
          ...
          <h3 className="text-[1.45rem] font-medium tracking-[-0.04em] text-[#F5F5F7]">
            {project.title}
          </h3>
